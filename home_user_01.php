...
<script>
  $(function () {
      // ===== FINAL JAVASCRIPT with return false; =====
      $('.sidebar-menu .treeview > a').on('click', function (e) {
          e.preventDefault();
      
          var $parent = $(this).parent('.treeview');
          
          // สลับ (เปิด/ปิด) เมนูที่คลิก
          $parent.toggleClass('active');
          $parent.find('.treeview-menu').first().stop(true).slideToggle(300);
          
          // ปิดเมนูอื่นที่ไม่ได้คลิก
          $('.sidebar-menu .treeview.active').not($parent).removeClass('active').find('.treeview-menu').stop(true).slideUp(300);
          
          // หยุดการทำงานของ event โดยสมบูรณ์ ป้องกันหน้าจอเลื่อน
          return false;
      });
      // =========================================================
