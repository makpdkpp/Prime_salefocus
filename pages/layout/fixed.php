...
<head>
  <meta charset="UTF-8">
  <title>ข้อมูลอุตสาหกรรม | PrimeFocus</title>
          ...
          <tbody>
            <?php while ($row = $result->fetch_assoc()): ?>
              <tr>
                ...
                <td>
                  <button class='btn btn-sm btn-info btn-edit' data-id='<?= $row['Industry_id'] ?>' data-name="<?= htmlspecialchars($row['Industry']) ?>" data-toggle='modal' data-target='#editModal'>
                    <i class='fa fa-edit'></i> Edit
                  </button>
                  <a href='delete_fixed.php?Industry_id=<?= $row['Industry_id'] ?>' onclick="return confirm('คุณต้องการลบหรือไม่?')" class='btn btn-sm btn-danger'>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
          <div class="modal-content">
            <form action="add01_product.php" method="POST">
              ...
              <div class="modal-header">
                <h5 class="modal-title">แก้ไขชื่ออุตสาหกรรม</h5>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
              </div>
              <div class="modal-body">
                <div class="form-group">
                  <label for="edit_industry">ชื่ออุตสาหกรรม:</label>
                  <input type="text" name="Industry" id="edit_industry" class="form-control" required>
                </div>
              </div>
              <div class="modal-footer">
...
<script>
  $(document).ready(function () {
    $('.btn-edit').on('click', function () {
      ...
      $('#edit_industry').val($(this).data('name'));
    });
  });
</script>
</body>
