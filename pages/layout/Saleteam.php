...
$totalQuery = $mysqli->query("SELECT COUNT(*) as total FROM team_catalog");
$totalRow = $totalQuery->fetch_assoc()['total'];
$totalPages = ceil($totalRow / $limit);

$sql = "SELECT team_id, team FROM team_catalog LIMIT $start, $limit";
...
<head>
  ...
  <title>ทีมขาย | PrimeForecast</title>
        ...
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item menu-is-opening menu-open">
            ...
            <ul class="nav nav-treeview">
              ...
              <li class="nav-item"><a href="../layout/Saleteam.php" class="nav-link active"><i class="fas fa-users nav-icon"></i><p>ทีมขาย</p></a></li>
  ...
  <div class="content-wrapper">
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">ข้อมูลทีมขาย</li>
            </ol>
          </div>
        </div>
      </div>
    ...
    <section class="content">
          ...
          <tbody>
            <?php while ($row = $result->fetch_assoc()): ?>
              <tr>
                ...
                <td>
                  <button class='btn btn-sm btn-info btn-edit' data-id='<?= $row['team_id'] ?>' data-name="<?= htmlspecialchars($row['team'], ENT_QUOTES) ?>">
                    <i class='fas fa-edit'></i> Edit
                  </button>
                  <a href='delete_team.php?team_id=<?= $row['team_id'] ?>' onclick="return confirm('คุณต้องการลบหรือไม่?')" class='btn btn-sm btn-danger'>
          ...
          <div class="modal-content">
            <form action="add_team.php" method="POST">
              <div class="modal-header">
                <h4 class="modal-title">เพิ่มชื่อทีมขาย</h4>
      ...
      <div class="modal fade" id="editModal" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
          <form id="editForm" action="add_team.php" method="POST">
            <input type="hidden" name="action" value="edit">
            <input type="hidden" name="team_id" id="edit_id">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title">แก้ไขชื่อทีมขาย</h5>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
              </div>
              <div class="modal-body">
                <div class="form-group">
                  <label for="edit_team">ชื่อทีมขาย:</label>
