...
// 4. teamEstimatevalue 
$sql = "
SELECT sum(transactional.product_value) AS teamEstimatevalue FROM transactional 
JOIN transactional_team
WHERE transactional_team.team_id = ?
";

$stmt = $mysqli->prepare($sql);
if (!$stmt) sendJsonError($mysqli->error);
$stmt->bind_param('i', $teamIDs);
if (!$stmt->execute()) sendJsonError($stmt->error);
$result = $stmt->get_result();
$output['teamEstimatevalue'] = $result->fetch_all(MYSQLI_ASSOC);
$stmt->close();
