...
$totalQuery = $mysqli->query("SELECT COUNT(*) as total FROM product_group");
$totalRow = $totalQuery->fetch_assoc()['total'];
$totalPages = ceil($totalRow / $limit);

$products = $mysqli->query("SELECT product_id, Product FROM product_group LIMIT $start, $limit");
?>
<!DOCTYPE html>
<html lang="th">
<head>
  ...
  <style>
    /* ✅ CSS ที่ปรับแก้ให้เหมือนหน้าก่อน */
        ...
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item menu-is-opening menu-open">
            ...
            <ul class="nav nav-treeview">
              ...
              <li class="nav-item"><a href="../layout/boxed.php" class="nav-link active"><i class="fas fa-boxes nav-icon"></i><p>เพิ่มข้อมูลกลุ่มสินค้า</p></a></li>
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">ข้อมูลกลุ่มสินค้า</li>
            </ol>
          </div>
        </div>
      </div>
          ...
          <tbody>
            <?php while ($p = $products->fetch_assoc()): ?>
              <tr>
                ...
                <td>
                  <button class='btn btn-sm btn-info btn-edit' data-id='<?= $p['product_id'] ?>' data-name="<?= htmlspecialchars($p['Product'], ENT_QUOTES) ?>">
                    <i class='fas fa-edit'></i> Edit
                  </button>
                  <a href='delete_boxed.php?product_id=<?= $p['product_id'] ?>' onclick="return confirm('ยืนยันการลบ?')" class='btn btn-sm btn-danger'>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title">เพิ่มข้อมูลกลุ่มสินค้า</h4>
  ...
  <div class="modal fade" id="editModal" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
      <form action="add1_product.php" method="POST">
        ...
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">แก้ไขชื่อกลุ่มสินค้า</h5>
            <button type="button" class="close" data-dismiss="modal">&times;</button>
          </div>
          <div class="modal-body">
            <div class="form-group">
              <label for="edit_product">ชื่อกลุ่มสินค้า:</label>
